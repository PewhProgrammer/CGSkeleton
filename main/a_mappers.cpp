...
void trymapperbonus(const char* filename, CoordMapper* spheremapper1, CoordMapper* spheremapper2) {
	static const float scale = 0.001f;
	Image img(400, 400);
	World world;
	SimpleGroup scene;
	...
	ImageTexture* fishTex = new ImageTexture("models/blue/screenShot.png");
	...
	//create environment map + emissive flat material
	EnvironmentCoordMapper env;
	...
	FlatMaterial mat(whitetex);
	scene.add(new Special(&env,&mat));
...
void a_mappers() {
	...
	trymapperbonus("map-6_environ.png",
		new SphericalCoordMapper(Point(.4f, .45f, .3f), Vector(0.0f, hsq2, hsq2), Vector(0.5f, 0.0f, 0.0f)),
		new SphericalCoordMapper(Point(.3f, .1f, .3f), Vector(0.0f, hsq2, -hsq2), Vector(0.5f, 0.0f, 0.0f))
	);
}
