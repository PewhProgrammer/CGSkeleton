...
namespace rt {
	...
	rt::SmoothTriangle::SmoothTriangle(Point vertices[3], Vector normals[3], CoordMapper * texMapper, Material * material)
	{
		A = vertices[0];
		B = vertices[1];
		C = vertices[2];
	...
	rt::SmoothTriangle::SmoothTriangle(const Point & v1, const Point & v2, const Point & v3, const Vector & n1, const Vector & n2, const Vector & n3, CoordMapper * texMapper, Material * material)
	{
		A = v1;
		B = v2;
		C = v3;
	...
	Intersection rt::SmoothTriangle::intersect(const Ray & ray, float previousBestDistance) const
	{
		...
		return Intersection(hit.distance, ray, this, normal, Point(u, v, w));
	}
